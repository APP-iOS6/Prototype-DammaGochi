...
struct StoreView: View {
                                ...
                                VStack {
                                    ...
                                    Button(action: {
                                        ...
                                        Text("구매하기")
                                            .font(.caption)
                                            .padding(5)
                                            .background(ownedProducts.contains(product.id) ? Color.gray : Color.purple)
            ...
            .alert(isPresented: $showAlert) {
                if let product = selectedProduct {
                    if userCoins >= product.price {
                        return Alert(
                            title: Text("구매 확인"),
                            message: Text("\(product.price) 코인을 사용하여 구매하시겠습니까?"),
    ...
    private func purchaseProduct(product: Product) {
        userCoins -= product.price
        ownedProducts.append(product.id)
        print("구매성공! 남은코인: \(userCoins)")
    }
}
